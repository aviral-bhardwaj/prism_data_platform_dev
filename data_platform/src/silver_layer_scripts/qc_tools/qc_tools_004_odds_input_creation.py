...
# output table name
odds_input_creation_table = 'qct_odds_input'
...
df_question = spark.table(f"{silver_catalog}.{canonical_tables_schema}.{dim_question_table}")
...
fact_nps = spark.table(f"{silver_catalog}.{canonical_tables_schema}.{fact_nps_table}").drop('gid_nps_fact')
...
fact_nps_enrich_translated_final = fact_nps_enrich_translated.withColumn("gid_odds", F.sha2(F.concat_ws('', *["gid_instrument", "gid_wave", "gid_respondent", "gid_provider", "gid_product", "gid_episode", "gid_channel", "gid_question_nps", "gid_question_verbatim", "typ_nps", "val_nps", "txt_verbatim"]), 256))    \
                                            .withColumnRenamed("verbatim_translated", "verbatim_translation")    \
...
if waves_to_delete.count() == 0:
    ...
    spark.sql(
        f"""
        DELETE FROM {silver_catalog}.{qc_tools_tables_schema}.{odds_input_creation_table}
        WHERE gid_wave IN ({str_waves_to_waves})
        """ )

    fact_nps_enrich_translated_final.write.mode('append').saveAsTable(f'{silver_catalog}.{qc_tools_tables_schema}.{odds_input_creation_table}')