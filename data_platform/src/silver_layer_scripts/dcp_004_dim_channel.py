...
df_answers_mapping = spark.sql(
    ...
    WHERE id_survey_question = 'nam_channel' and txt_answer is not null
    """
)
...
df_channel_final = (
    df_answer_long
    .withColumnRenamed('txt_answer', 'nam_channel')\
        .withColumnRenamed('recoded_txt_answer', 'nam_channel_disp')\
    .select('gid_instrument', 'nam_channel','nam_channel_disp')
    .dropDuplicates(['gid_instrument', 'nam_channel'])
)
...
if spark.catalog.tableExists(f"{silver_layer}.{canonical_tables_catalog}.{dim_channel_table}"):

    dict_df = {dim_channel_table: df_channel_final}
    key_cols = {dim_channel_table: ['nam_channel','gid_instrument']}
    ...
    if df_new_records.isEmpty():
        ...
        df_new_records.select(spark.table(f"{silver_layer}.{canonical_tables_catalog}.{dim_channel_table}").columns)  \
            .write.format("delta")  \
            .mode("append") \
            .option("mergeSchema", "true")   \
            .saveAsTable(f"{silver_layer}.{canonical_tables_catalog}.{dim_channel_table}")
...
# Else save new table
    ...
    df_channel_final = df_channel_final.withColumn("gid_channel", F.row_number().over(Window.orderBy("nam_channel")))
