...
df_attributes = (
    df_gold_mapping
    .filter((F.col('mapping_category')=='KPC Attributes') & (F.col('survey_id')==777777))
...
df_fact_final = (
    df_fact_response.drop('gid_instrument', 'dte_create')
    .join(df_dim_question, on=['gid_question'], how='left')
    .where((F.col('desc_question_grp')=='uid_KPC_rating') & (F.col('gid_instrument')==2))
    .withColumn('Loyalty program code', F.regexp_replace('id_survey_question', 'Q_Loyalty_Program_Attributes_1_', ''))
    .withColumn('Top 2', 
